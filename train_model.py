...
def test(model, test_loader, criterion, debug_hook):
    '''
...
def train(model, train_loader, val_loader, criterion, optimizer, epochs, debug_hook):
    '''
        ...
        for type in ['train', 'val']:
            ...
            if type == 'val':
                if accuracy > best_accuracy:
                    ...
                    # reason to keep going is so the debugger can collect more tensors
                    print(f'Accuracy has not improved from the previous epoch {epoch-1}, we will keep going!')
...
def main(args):
    ...
    # create debug hook and register to the model and loss function
    debug_hook = smd.Hook.create_from_json_file()
    debug_hook.register_hook(model)
    debug_hook.register_loss(loss_criterion)
    ...
    model = train(model, data_loaders['train'], data_loaders['val'], loss_criterion, optimizer, int(args.epochs), debug_hook)

    '''
    TODO: Test the model to see its accuracy
    '''
    test(model, data_loaders['test'], loss_criterion, debug_hook)

    '''
    TODO: Save the trained model
    '''
    torch.save(model.state_dict(), os.path.join(args.model_dir, 'model.pt'))
    print('saved model params to model.pt')
...
if __name__=='__main__':
    ...
    print('train_model.py was called with these parameters: ')
    print(args)
